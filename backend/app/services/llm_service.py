...
镜头{start_shot}，镜头景别：xx，拍摄角度：xx，运镜方式：xx，画面主体人物：xx，场景环境：xx，情绪氛围：xx，构图：xx，画质补充：金属冷光，发丝清晰，轮廓与表面材质细节完整，抗锯齿高清渲染，转场衔接：xx，镜头时长：N秒
...
10. 若角色当前处于特殊着装状态（赤裸、裸露上身、贴身、缠绷带、湿身、披挂等），画面主体人物字段必须显式写出该状态词（如「赤裸上身」「光着上身」「缠满绷带」），不可省略，不可依赖前文或角色默认服装
11. 不要输出任何其他文字"""
...
镜头N，镜头景别：值，拍摄角度：值，运镜方式：值，画面主体人物：详细描述，场景环境：详细描述，台词对白：角色名：台词或@无，情绪氛围：两字，构图：值，画质补充：金属冷光，发丝清晰，轮廓与表面材质细节完整，抗锯齿高清渲染，转场衔接：值，镜头时长：M秒
...
14. 【着装状态显式】若角色当前处于特殊着装状态（赤裸上身、裸露、光着上身、贴身、缠绷带、缠满绷带、湿身、披挂、袒露等），画面主体人物字段必须显式写出该状态词（如「赤裸上身」「光着上身」「缠满绷带」），绝不可省略或依赖前一个镜头；同一角色在不同镜头可有不同着装状态，必须以本镜头描述为准

## 转场衔接（必须服务于叙事）
15. 转场衔接必须根据相邻镜头之间的剧情关系和情绪变化来选定，选项及适用场景：
    · 淡入 —— 全片第一个镜头，开场专用
    · 硬切 —— 同一场景内快速切换，保持紧张节奏
...
16. 转场与场景变化匹配：场景不变→硬切或匹配剪辑；场景切换→溶解或推入；情绪断点→黑场过渡；回忆/闪回→闪白或模糊过渡
17. 不同叙事段落之间的转场必须使用有叙事标识的类型（黑场过渡/溶解/推入），让观众感知到剧情阶段推进

## 最终约束
18. 只输出 TOTAL_SHOTS行 + GLOBAL_PREFIX行 + N个镜头行 + POST_CONSTRAINT行，不要任何其他文字
19. 你声明的 TOTAL_SHOTS 必须严格等于下方输出的镜头行数，不一致说明你工作不认真"""
...
- 每个角色的 portrait_prompt 必须干净背景、正面半身定妆、五官/发型/服装细节明确，且服装措辞与 description 里的「服装」字段保持一致，用于后续分镜图「逐字锁定」外观。
