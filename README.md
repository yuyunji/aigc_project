...
摄影与视觉要求 · 衔接与结尾桥接）+ 导演阐述。模板固化在 skill `director-storyboard`，
运行时真源为 `backend/app/services/director_storyboard_skill.py`。
...
| LLM 拆解链路（导演镜头拆解） | 支付 / 会员体系 |
| 异步任务队列（内存模拟） | Redis / RabbitMQ 等中间件 |
| 任务状态管理 + 进度轮询 | 多用户并发管理 |
| 结果预览（导演镜头脚本/资产拆解） | 企业级权限系统 |
| 极简统计看板 | CI/CD / K8s |
